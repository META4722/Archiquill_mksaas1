...
interface ExteriorDisplayProps {
  images: string[];
  timing: number | null;
  isLoading: boolean;
}

export function ExteriorDisplay({ images, timing, isLoading }: ExteriorDisplayProps) {
  const t = useTranslations('AIExteriorPage.display');

  const handleDownload = (imageUrl: string, index: number) => {
    const a = document.createElement('a');
    a.href = imageUrl;
    a.download = `exterior-design-${Date.now()}-${index + 1}.png`;
    a.target = '_blank';
