...
import { useExteriorGeneration } from '../hooks/use-exterior-generation';
import { ImageUpload } from './ImageUpload';
import { ExteriorDisplay } from './ExteriorDisplay';
import { SignupPromptDialog } from '../../garden/components/SignupPromptDialog';

type GenerationMode = 'image_to_image' | 'text_to_image';

const PENDING_PROMPT_KEY = 'archiquill_exterior_pending_prompt';

export function ExteriorPlayground() {
  const t = useTranslations('AIExteriorPage');
  const router = useRouter();
  const { data: session, isPending: sessionPending } = authClient.useSession();
  const isLoggedIn = !!session?.user;

  const { images, isLoading, error, timing, generateExterior } = useExteriorGeneration();
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-2 lg:gap-12">
      ...
      <div className="lg:sticky lg:top-6 lg:h-fit">
        <ExteriorDisplay images={images} timing={timing} isLoading={isLoading} />
      </div>
